...
Of these incidents, "Narcotic/Drug Law Violations accounted for 2,551.
...
ggplot(philly.crime, aes(x = fct_rev(fct_infreq(text_general_code)),
                         fill=ifelse(text_general_code == "Narcotic / Drug Law Violations" ,"Highlighted","Others"))) +
  geom_bar() +
  labs(
  ...
  coord_flip() + 
  scale_fill_manual(values = c("Highlighted" = "#336699", "Others" = "gray")) +
  theme_minimal() +
...
This assignment examines whether there is a spatial correlation between locations of Narcotic / Drug Violations incidents occurred and distance of police stations and locations of parks and public recreation places, and use this to create a predictive model.
The following map shows the locations of police stations and districts, and Public Parks and Recreations (PPR) places' locations in Philadelphia. Areas farther from police stations may give criminals a sense of being less monitored, potentially leading to a higher frequency of drug incidents. Some areas with active drug trade may be relatively close to police stations, indicating that the presence of police is not always enough to deter narcotic crimes. 
Parks are often large, and the view is easily obstructed by trees or structures, making it possible for discreet activities to occur. Some parks may have gaps in security during nighttime hours. This could increase the likelihood of parks being chosen as locations for Narcotics / Drugs Violations.
...
vars_net <- rbind(philly.policestn, philly.PPR) %>%
...
The results firstly confirm that the simple counts and Local Moran's I are concentrated in the Kensington area. Second, the p-values show that the Kensington area and some areas around the police station have high p-values, while the other areas are not statistically significant.
...
reg.vars <- c("policeStn.nn","PPR.nn")
## define the variables we want
reg.ss.vars <- c("policeStn.nn","PPR.nn","narcotic.isSig","narcotic.isSig.dist")
...
In addition, in the risk prediction model in the following plots, the 2nd and 3rd were over-estimated, and the 4th and 5th were under-estimated compared to the KD model. However, on the contrary, a well-fit model should show that the risk prediction captures a greater share of 2024 in the highest risk category relative to the kernel density. 
...
In this assignment, a geospatial risk model for Narcotics / Drugs Crimes in Philadelphia in 2023 was produced based on the locations of police stations and public parks and recreation places, as well as LOGO cross-validation and kernel density analysis for the model, and comparison with application to the 2024 data. The analysis revealed that the Kensington area formed a significant hotspot for narcotic crimes. However, it was unclear how much the locations of police stations and parks contributed to the model, likely because the narcotic violations data in Philadelphia were concentrated in one area known as Kensington. For the same reason, it was difficult to predict the future spatial spread of drug-related crimes based on the Broken Windows Theory. In the future, given the importance of crime prevention, a spatial analysis approach with the efforts to incorporate additional factors such as police patrol routes, ports, airports, major roads, and traffic information must be necessary.
